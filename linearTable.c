...
//返回：返回状态
Status ListInsert(sqList *L1, int i, ElemType e)
{
	sqList L = *L1;
	ElemType *newbase = L.elem;
...
//返回：返回状态
Status ListDelete(sqList *L1, int i, ElemType *e)
{
	sqList L = *L1;
	ElemType *p,*q;
	if (i<1 || i>L.length + 1) return ERROR;
...
int LocateElem(sqList *L1, ElemType e, Status(*compare)(ElemType, ElemType))
{
	sqList L = *L1;
	ElemType *q, *p;
	int count = 1;
...
void MergeList(sqList *L1_in, sqList *L2_in, sqList *L3)
{
	...
	sqList L1 = *L1_in;
	sqList L2 = *L2_in;
	ElemType *p,*p_end, *q,*q_end, *r,*r_end;
	p = &L1.elem[0];
